...
static partial class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        {
            try { CheckForegroundGuard(); CheckPerformanceGate(); CheckNavigationJournal(); CheckWebViewEnvironment(); return 0; }
            catch (Exception error) { Console.Error.WriteLine(error); return 1; }
        }
    ...
    static async Task RunAsync()
    {
        ...
        try
        {
            ...
            string[] performanceFailures = PerformanceFailures(readyIdle, baseline, idle, repeated);
            ...
            Console.WriteLine(JsonSerializer.Serialize(new
            {
                ...
                warm_working_set_growth_bytes = repeated.working_set_bytes - idle.working_set_bytes,
                warm_private_growth_bytes = repeated.private_bytes - idle.private_bytes,
                lifecycle_pass = true,
    ...
    static string[] PerformanceFailures(IdleSample ready, IdleSample dashboard, IdleSample warm, IdleSample repeated)
    {
        ...
        foreach (var (name, sample, memoryLimit) in new[] { ("dashboard-ready", ready, 600_000_000L),
            ("dashboard-routes", dashboard, 600_000_000L), ("warm", warm, 1_073_741_824L), ("repeated", repeated, 1_073_741_824L) })
        {
        ...
        if (repeated.working_set_bytes - warm.working_set_bytes > 16_777_216) failures.Add("warm working-set growth exceeds 16 MiB");
        if (repeated.private_bytes - warm.private_bytes > 16_777_216) failures.Add("warm private-commit growth exceeds 16 MiB");
        if (!warm.processes.Select(x => (x.pid, x.start_time_utc)).Order()
            ...
            failures.Add("warm process identities/count changed across repeated route cycle");
    ...
    static void CheckPerformanceGate()
    {
        ...
        Check(PerformanceFailures(sample, sample, sample, sample).Length == 0, "exact CPU ceiling must pass");
        Check(PerformanceFailures(sample, sample with { cpu_seconds = 0.250001 }, sample, sample).Length > 0,
            "aggregate CPU over budget was accepted");
        Check(PerformanceFailures(sample with { working_set_bytes = 600_000_000 }, sample, sample, sample).Length > 0,
            "dashboard memory ceiling was not strict");
        Check(PerformanceFailures(sample, sample, sample with { working_set_bytes = 1_073_741_824 }, sample).Length > 0,
            "warm memory ceiling was not strict");
        Check(PerformanceFailures(sample, sample, sample, sample with { working_set_bytes = 516_777_216,
            private_bytes = 216_777_216 }).Length == 0, "exact growth ceiling must pass");
        Check(PerformanceFailures(sample, sample, sample, sample with { working_set_bytes = 516_777_217 }).Length > 0,
            "working-set growth over budget was accepted");
        Check(PerformanceFailures(sample, sample, sample, sample with { private_bytes = 216_777_217 }).Length > 0,
